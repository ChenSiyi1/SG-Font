...
class TrainLoop:
    ...
    def run_loop(self):
        if self.classifier_free:
            while (
                    self.step + self.resume_step < self.total_train_step # total
            ...
            while (
                    self.step + self.resume_step < self.train_step # train_step
                    and (not self.lr_anneal_steps
                         or self.step + self.resume_step < self.lr_anneal_steps)
            ):
                batch, cond = next(self.data)
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            micro_cond['source'] = cont_batch[i: i + self.microbatch].to(dist_util.dev())
