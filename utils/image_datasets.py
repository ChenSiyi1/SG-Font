...
def load_data(
    *,
    ...
    source_files = _list_image_files_recursively(source_data_dir)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.local_classes is not None:
            ...
            out_dict["mask_sty"] = False

            cont_img_idx = self.local_classes[idx]
            cont_img_path = self.source_images[cont_img_idx]
            with bf.BlobFile(cont_img_path, "rb") as f:
                cont_pil_image = Image.open(f)
                cont_pil_image.load()
            cont_pil_image = cont_pil_image.convert("RGB")
            cont_arr = center_crop_arr(cont_pil_image, self.resolution)
            cont_arr = cont_arr.astype(np.float32) / 127.5 - 1
        ...
        if self.style_classes is not None:
            ...
            return [np.transpose(arr, [2, 0, 1]), np.transpose(sty_arr, [2, 0, 1]),
                    np.transpose(cont_arr, [2, 0, 1])], out_dict  # 原图，随机同种风格的图片，[y,stroke]
...
class ImageDataset_Clsfree(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.local_classes is not None:
            out_dict["y"] = np.array(self.local_classes[idx], dtype=np.int64)
            out_dict["mask_y"] = random.random() < 0.2  # 0.2
            out_dict["mask_sty"] = random.random() < 0.2
            cont_img_idx = self.local_classes[idx]  # 同种内容的字的索引
            cont_img_path = self.source_images[cont_img_idx]
            with bf.BlobFile(cont_img_path, "rb") as f:
                cont_pil_image = Image.open(f)
                cont_pil_image.load()
        ...
        if self.style_classes is not None:
            ...
            return [np.transpose(arr, [2, 0, 1]), np.transpose(sty_arr, [2, 0, 1]),
                    np.transpose(cont_arr, [2, 0, 1])], out_dict
        return np.transpose(arr, [2, 0, 1]), out_dict
